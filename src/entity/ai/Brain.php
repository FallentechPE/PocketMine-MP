...
use pocketmine\entity\ai\behavior\BehaviorStatus;
use pocketmine\entity\ai\memory\ExpirableValue;
use pocketmine\entity\ai\memory\MemoryModuleType;
use pocketmine\entity\ai\memory\MemoryStatus;
use pocketmine\entity\ai\sensing\Sensor;
use pocketmine\entity\Mob;
use pocketmine\entity\schedule\Activity;
use pocketmine\entity\schedule\Schedule;
...
class Brain {
	...
	public function getRunningBehaviors() : array {
		$behaviors = [];
		foreach ($this->availableBehaviorsByPriority as $activities) {
			foreach ($activities as $behaviors) {
	...
	public function updateActivityFromSchedule(int $dayTime, int $gameTime) : void {
		if ($gameTime - $this->lastScheduleUpdate > 20) {
			...
			$activity = $this->getSchedule()->getActivityAt($dayTime % 24000);
	...
	private function tickSensors(Mob $entity) : void {
	...
	public function stopAll(Mob $entity) : void {
	...
	private function startEachNonRunningBehavior(Mob $entity) : void {
		$time = $entity->getWorld()->getServer()->getTick();
		foreach ($this->availableBehaviorsByPriority as $activities) {
			foreach ($activities as $behaviors) {
				foreach ($behaviors as $behavior) {
	...
	private function tickEachRunningBehavior(Mob $entity) : void {
		$time = $entity->getWorld()->getServer()->getTick();
		foreach ($this->getRunningBehaviors() as $behavior) {
