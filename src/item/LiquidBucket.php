...
class LiquidBucket extends Item{
	...
	public function onInteractBlock(Player $player, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, array &$returnedItems) : ItemUseResult{
		...
		if(!$ev->isCancelled()){
			...
			}elseif ($blockReplace instanceof Waterloggable && $resultBlock instanceof Water) {
				$blockReplace->setWaterState($resultBlock);
				$player->getWorld()->setBlock($blockReplace->getPosition(), $blockReplace);
				$blockReplace->onNearbyBlockChange();
			} else {
