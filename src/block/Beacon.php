...
final class Beacon extends Transparent{
	...
	]; // todo It would be nice to allow plugins to be able to modify this
	...
	public function onScheduledUpdate() : void{
		$this->position->getWorld()->scheduleDelayedBlockUpdate($this->position, 20 * 3); // todo According to the wiki, updates are every 4 seconds

		if($this->primaryEffect === null){
			return;
		}
		if(($beaconLevel = $this->getBeaconLevel()) >= self::MIN_LEVEL){
			if(!$this->viewSky()){
				return;
			}
		} // todo We need some validation here, it is possible that players might destroy blocks on any level and some effects will be no longer avaible
	...
	public function isBeaconLevelValid(int $level) : bool{ // todo Please document this function, i feel that the name isn't clear enough
		if($level < self::MIN_LEVEL || $level > self::MAX_LEVEL){
			throw new InvalidArgumentException("Beacon level must be in range " . self::MIN_LEVEL . "-" . self::MAX_LEVEL . ", $level given");
		}
	...
	public function viewSky() : bool{ // todo The function name is not the most appropriate.
		$world = $this->position->getWorld();
		$maxY = $world->getMaxY();
		$block = $this;
		for($y = 0; $y <= $maxY; $y++){
	...
	public function getAllowedEffects(int $beaconLevel) : array{
		if($beaconLevel < self::MIN_LEVEL || $beaconLevel > self::MAX_LEVEL){
			throw new InvalidArgumentException("Beacon level must be in range " . self::MIN_LEVEL . "-" . self::MAX_LEVEL . ", $beaconLevel given");
		}
		$levelEffects = $this->getLevelEffect();
		$allowed = [];
		for($i = self::MIN_LEVEL; $i <= $beaconLevel; $i++){
